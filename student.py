...
class Student:
    def __init__(self, root):
        ...
        # left frame
        left_frame = LabelFrame(main_frame, bd=2, relief=RIDGE, text="Student Details", cursor="hand2",
                                font=("times new roman", 12, "bold"), bg="white", fg="red")
        left_frame.place(x=10, y=10, width=630, height=540)
        img_left = Image.open(r"blue.jpg")
        img_left = img_left.resize((100, 60), Image.Resampling.LANCZOS)  # resize and converting high level img to low level img.
        self.photoimg_left = ImageTk.PhotoImage(img_left)
        f_lbl = Label(left_frame, image=self.photoimg_left, cursor="hand2")
        f_lbl.place(x=270, y=10, width=60, height=60)

        # current course
        current_course_frame = LabelFrame(left_frame, relief=RIDGE, text="Current Course Information",
        ...
        dep_combo["values"] = (
        "Select Department", "Computer Science", "Information Science ", "Electronics", "Mechanical")
        dep_combo.current(0)
        ...
        "Select Semester", "First", "Second", "Third", "Fourth", "Fifth", "Sixth", "Seventh", "Eighth")
        semester_combo.current(0)
        semester_combo.grid(row=1, column=1, padx=2, pady=10, sticky=W)

        # Class Information
        class_Student_frame = LabelFrame(left_frame, relief=RIDGE, text="Class Information",
                                         font=("times new roman", 12, "bold"), fg="red")
        ...
        studentName_entry = ttk.Entry(class_Student_frame, textvariable=self.var_std_name, width=15,
                                      font=("times new roman", 12, "bold"))
        studentName_entry.grid(row=0, column=3, padx=10, pady=5, sticky=W)

        # Class Division
        class_div_label = Label(class_Student_frame, text="Class and Section:", font=("times new roman", 12, "bold"),
                                fg="red")
        class_div_label.grid(row=1, column=0, padx=10, sticky=W)

        class_div_entry = ttk.Entry(class_Student_frame, textvariable=self.var_div, width=15,
                                    font=("times new roman", 12, "bold"))
        ...
        roll_no_entry = ttk.Entry(class_Student_frame, textvariable=self.var_roll, width=15,
                                  font=("times new roman", 12, "bold"))
        ...
        dob_entry = ttk.Entry(class_Student_frame, textvariable=self.var_dob, width=15,
                              font=("times new roman", 12, "bold"))
        ...
        email_entry = ttk.Entry(class_Student_frame, textvariable=self.var_email, width=15,
                                font=("times new roman", 12, "bold"))
        ...
        phone_entry = ttk.Entry(class_Student_frame, textvariable=self.var_phone, width=15,
                                font=("times new roman", 12, "bold"))
        ...
        address_entry = ttk.Entry(class_Student_frame, textvariable=self.var_address, width=15,
                                  font=("times new roman", 12, "bold"))
        ...
        teacher_entry = ttk.Entry(class_Student_frame, textvariable=self.var_teacher, width=15,
                                  font=("times new roman", 12, "bold"))
        teacher_entry.grid(row=4, column=3, padx=10, pady=5, sticky=W)

        # radio buttons
        self.var_radio1 = StringVar()
        radionbtn1 = ttk.Radiobutton(class_Student_frame, variable=self.var_radio1, text="Take a photo sample",
                                     value="Yes")
        ...
        # take photo button
        take_photo_btn = Button(btn_frame1, command=self.generate_dataset, text="Take Photo Sample", width=30,
                                font=("times new roman", 12, "bold"), fg="white", bg="red")
        take_photo_btn.grid(row=0, column=0, padx=10, pady=2, sticky=W)

        # update photo button
        update_btn = Button(btn_frame1, text="Update Photo", command=self.generate_dataset, width=30,
                            font=("times new roman", 12, "bold"), fg="white", bg="red")
        update_btn.grid(row=0, column=1, padx=2, pady=2, sticky=W)

        # right frame
        right_frame = LabelFrame(main_frame, bd=2, relief=RIDGE, text="Student Details", cursor="hand2",
                                 font=("times new roman", 12, "bold"), bg="white", fg="red")
        right_frame.place(x=660, y=10, width=630, height=540)

        img_right = Image.open(r"blue.jpg")
        img_right = img_right.resize((250, 60),
        ...
        # =======SEARCH SYSTEM=======
        search_frame = LabelFrame(right_frame, relief=RIDGE, text="Search System (UNDER CONSTRUCTION)",
                                  font=("times new roman", 12, "bold"), fg="red")
        ...
        search_btn = Button(search_frame, text="Search", width=10, font=("times new roman", 12, "bold"), fg="white",
                            bg="red")
        search_btn.grid(row=0, column=3, padx=3, pady=4, sticky=W)

        showAll_btn = Button(search_frame, text="Show All", width=10, font=("times new roman", 12, "bold"), fg="white",
                             bg="red")
        ...
        "Department", "Year", "Semester", "ID", "Name", "Division", "Roll No", "Gender", "DOB", "Email", "Phone",
        "Address", "Teacher", "Photo Sample Status"), xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)
    ...
    def add_data(self):
        if self.var_dep.get() == "Select Department" or self.var_std_name.get() == "" or self.var_std_id.get() == "":
            ...
            try:
                ...
                self.var_dep.get(), self.var_year.get(), self.var_semester.get(), self.var_std_id.get(),
        ...
        if self.var_dep.get() == "Select Department" or self.var_std_name.get() == "" or self.var_std_id.get() == "":
            ...
            try:
                ...
                if Update > 0:
                    ...
                    my_cursor = conn.cursor()
                    my_cursor.execute(
                        '''Update Student SET Department=?, Year=? , Semester=? , Name=? , Division=? , Roll=? , Gender=? , DOB=? , Email=? , Phone=? , Address=? , Teacher=? , PhotoSample=? where ID=?''',
                        (self.var_dep.get(), self.var_year.get(), self.var_semester.get(), self.var_std_name.get(),
                         self.var_div.get(), self.var_roll.get(), self.var_gender.get(), self.var_dob.get(),
                         self.var_email.get(), self.var_phone.get(), self.var_address.get(), self.var_teacher.get(),
    ...
    def generate_dataset(self):
        ...
        name= self.var_std_id.get()

        if self.var_dep.get() == "Select Department" or name == "" or self.var_std_id.get() == "":
            messagebox.showerror("ERROR", "All fields are required", parent=self.root)
        else:
            try:
                conn = sqlite3.connect('FR.db')
                ...
                my_cursor.execute(
                    '''Update Student SET Department=?, Year=? , Semester=? , Name=? , Division=? , Roll=? , Gender=? , DOB=? , Email=? , Phone=? , Address=? , Teacher=? , PhotoSample=? where ID=?''',
                    (self.var_dep.get(), self.var_year.get(), self.var_semester.get(), name,
                     self.var_div.get(), self.var_roll.get(), self.var_gender.get(), self.var_dob.get(),
                     self.var_email.get(), self.var_phone.get(), self.var_address.get(), self.var_teacher.get(),
                     self.var_radio1.get(), self.var_std_id == id + 1,))
                conn.commit()
                self.fetch_data()
                ...
                while True:
                    ...
                    for (x, y, w, h) in faces:
                        ...
                        cv2.imwrite("data/User." + str(id) +"." + name + "." + str(img_id) + ".jpg", gray[y:y + h, x:x + w])
